...
interface HouseholdDao {
    ...
    suspend fun addTransactionWithAudit(transaction: Transaction, ledgerEvent: LedgerEventEntity) {
        insertTransaction(transaction); updateWalletBalance(transaction.walletId, transaction.amount); insertLedgerEvent(ledgerEvent)
    }

    @DbTransaction
    suspend fun deleteTransactionWithAudit(transaction: Transaction, ledgerEvent: LedgerEventEntity) {
        softDeleteTransaction(transaction.id); updateWalletBalance(transaction.walletId, -transaction.amount); insertLedgerEvent(ledgerEvent)
    }

    @DbTransaction
    suspend fun updateTransactionWithAudit(oldTx: Transaction, newTx: Transaction, ledgerEvent: LedgerEventEntity) {
        updateWalletBalance(oldTx.walletId, -oldTx.amount); updateWalletBalance(newTx.walletId, newTx.amount); insertTransaction(newTx); insertLedgerEvent(ledgerEvent)
    }

    @DbTransaction
    suspend fun executeAtomicTransferWithAudit(
        debitTx: Transaction, creditTx: Transaction, transferEvent: TransferEventEntity,
        debitLedgerEvent: LedgerEventEntity, creditLedgerEvent: LedgerEventEntity
    ) {
        insertTransaction(debitTx); insertTransaction(creditTx)
        updateWalletBalance(debitTx.walletId, debitTx.amount); updateWalletBalance(creditTx.walletId, creditTx.amount)
        insertTransferEvent(transferEvent); insertLedgerEvent(debitLedgerEvent); insertLedgerEvent(creditLedgerEvent)
    }
