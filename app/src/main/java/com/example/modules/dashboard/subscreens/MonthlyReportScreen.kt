...
fun MonthlyReportScreen(
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = {
                        ...
                        val husbandExp = monthData.transactions.filter { it.memberId == (husband?.id ?: "") && it.amount < 0 }.sumOf { -it.amount }
                        val wifeExp = monthData.transactions.filter { it.memberId == (wife?.id ?: "") && it.amount < 0 }.sumOf { -it.amount }
                        val summaryText = "📊 *LAPORAN KEUANGAN KELUARGA TRANSPARAN*\n" +
                                ...
                                "• ${husband?.name ?: "Suami"}: ${formatter.format(husbandExp)}\n" +
                                "• ${wife?.name ?: "Istri"}: ${formatter.format(wifeExp)}\n\n" +
        ...
        LazyColumn(
            ...
            item {
                TransparencyHealthCard(
                    ...
                    transactionCount = monthData.transactions.size
                )
            }
            item {
    ...
    if (showEditBudgetDialog) {
        EditBudgetDialog(
            ...
            onConfirm = { onUpdateBudget(it); showEditBudgetDialog = false },
